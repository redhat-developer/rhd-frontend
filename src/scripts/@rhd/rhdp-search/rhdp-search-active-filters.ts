...
import RHDPSearchFilterActiveItem from './rhdp-search-filter-active-item.js';

export default class RHDPSearchActiveFilters extends PFElement {
    get html() {
        return `
        <style>
            :host {
    ...
    static get tag() { return 'rhdp-search-active-filters'; }
    ...
    constructor() {
        super(RHDPSearchActiveFilters, {delayRender: true});
    ...
    _checkActive(e) {
        if (e.detail) {
            let chk = top.document.querySelectorAll('rhdp-search-filter-item[active]');
            if (chk.length > 0) {
    ...
    _addFilters() {
        ...
        for(let i=0; i < groups.length; i++) {
            ...
            for(let j=0; j < items.length; j++) {
                let item = new RHDPSearchFilterActiveItem();
...
PFElement.create(RHDPSearchActiveFilters);