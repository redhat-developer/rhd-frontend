...
export default class RHDPSearchFilters extends HTMLElement {
    constructor() {
        ...
        this._toggle = false;
        this.modalTemplate = (string, title) => {
        ...
        this.activeTemplate = (strings, title) => {
    ...
    addGroups() {
        let groups = this.filters.facets, len = groups.length;
        for (let i = 0; i < len; i++) {
            let group = new RHDPSearchFilterGroup(), groupInfo = groups[i], groupNode = group.querySelector('.group'), primaryFilters = group.querySelector('.primary'), secondaryFilters = group.querySelector('.secondary'), len = groupInfo.items ? groupInfo.items.length : 0;
            if (len <= 5) {
